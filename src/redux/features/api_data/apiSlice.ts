...
export const fetchRecipes = createAsyncThunk(
    ...
    async (type: string) => {
        ...
        const searchResponse = await fetch(
            `https://api.spoonacular.com/recipes/complexSearch?query=${type}&apiKey=${import.meta.env.VITE_SPOONACULAR_TOKEN2}`
        );
        ...
        const recipesPromises = searchResult.results.map(async (result: any) => {
            const response = await fetch(
                `https://api.spoonacular.com/recipes/${result.id}/information?apiKey=${import.meta.env.VITE_SPOONACULAR_TOKEN2}`
            );
            if (!response.ok) {
